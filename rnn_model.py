...
class RNNModel:
    def __init__(
        ...
        with tf.variable_scope("rnn", reuse=tf.AUTO_REUSE):
            softmax_layer = tf.get_variable(
                "softmax_layer", [hidden_layer_size, vocabulary_size]
            )
            softmax_bias = tf.get_variable("softmax_bias", [vocabulary_size])

        with tf.variable_scope("embedding", reuse=tf.AUTO_REUSE):
            embedding = tf.get_variable(
                "embedding", [vocabulary_size, hidden_layer_size]
            )
        ...
        grads, _ = tf.clip_by_global_norm(
            tf.gradients(self.cost, trainable_vars), gradient_clip
        )
    ...
    def sample(self, sess, chars, vocabulary, length):
        ...
        for _ in range(length):
            ...
            [probabilities, state] = sess.run(
                [self.probabilities, self.final_state], feed
            )
